...
export const computeBuildLocation = ({
	initialLocation,
	buildDirection,
	trackElementType,
}: {
...
const getAdjustedBeginZ = ({
	...
	return {
		// building next & flat/up, or previous & flat/down
		beginZ: 0,
	};
};
 ...
 */
const getSegmentBeginAndEndZ = (
	segmentType: TrackElementType
): { beginZ: number; endZ: number } => {
	const thisSegment = context.getTrackSegment(Number(segmentType));
	return thisSegment
		? { beginZ: thisSegment.beginZ, endZ: thisSegment.endZ }
		: { beginZ: 0, endZ: 0 };
};
...
const getAdjustedXYCoords = ({
	...
	if (thisTrackType.beginDirection != thisTrackType.endDirection) {
		// an attempt to fix diagonal tracks
