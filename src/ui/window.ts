...
import { buttonState, segmentState, trackElementsOnSelectedTile } from "~/stores";
import selectSegment from "~/ui/buttons/actions/selectSegmentAction";
import { customImageFor } from "~/ui/customImages/customButtonSprites";
import debugComponents from "~/ui/debugComponents";
...
let title = `Advanced Build Menu`;

export const trackIteratorWindow = (): WindowTemplate => {
	// get all main state variable accessible.

	// create the element wrapper
	const element = new ElementWrapper();
	...
	return window({
		...
		content: [
					...
					horizontal({
						content: [
							...
							element.toggle({
								...
								isPressed: compute(
									buttonState.pressedButtons.select,
									(isPicking) => isPicking
								),
			...
			// choose which segment from the selected tile
			dropdown({
				items: compute(trackElementsOnSelectedTile, (elements) =>
					elements.map((e) =>
						e
							? `Ride: ${e.element.ride}, height: ${e.element.baseHeight}, i: ${
									TrackElementType[<TrackElementType>e.segment?.trackType || 0]
							  }`
							: `Ride is null.`
					)
				),
				onChange: (selectedIndex) => {
					segmentState.updateSegmentSequence(
						trackElementsOnSelectedTile.get()[selectedIndex]?.segment
					);
				},
				selectedIndex: compute(segmentState.selectedSegment, (segment) => {
					const potentialIndexOf = trackElementsOnSelectedTile
						.get()
						.map((tei) => tei.segment)
						.indexOf(segment);
			...
			debugComponents.buttonPressListview(),
			debugComponents.selectedSegmentListview(),
